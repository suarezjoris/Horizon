...
window.updateIdeStatus = (status, icon = '💤') => {
    ...
    if (indicator) {
        ...
        indicator.style.color = (status.includes('Executing') || status.includes('Working'))
            ? 'var(--accent-gold)'
            : 'var(--text-dim)';
    }
};
...
window.ideSendPrompt = async () => {
    ...
    try {
        await window.__TAURI__.core.invoke('send_ide_prompt', { prompt: text, mode: 'fast' });
    } catch (e) {
...
window.ideAppendMessage = (text, isUser, isError = false, isSystem = false) => {
    ...
    if (isUser) div.classList.add('ide-message-user');
    else if (isSystem) div.classList.add('ide-message-system');
    else if (isError) div.classList.add('ide-message-error');
    else div.classList.add('ide-message-ai');

    if (isSystem || isError) {
        div.textContent = text;
    } else {
        div.innerHTML = (window.marked && window.DOMPurify)
            ? window.DOMPurify.sanitize(window.marked.parse(text))
            : `<pre style="white-space:pre-wrap;word-break:break-all">${text.replace(/</g, '&lt;')}</pre>`;
    }
...
async function setupIdeListeners() {
    ...
    await listen('ide-status', (event) => {
        ...
        if (event.payload.model) {
            const el = document.getElementById('ide-model-indicator');
            if (el) el.textContent = '🧠 ' + event.payload.model;
        }
    });

    await listen('ide-response', (event) => {
        if (currentStreamDiv) {
            currentStreamDiv.innerHTML = (window.marked && window.DOMPurify)
                ? window.DOMPurify.sanitize(window.marked.parse(currentStreamDiv.textContent))
                : `<pre style="white-space:pre-wrap;word-break:break-all">${currentStreamDiv.textContent.replace(/</g, '&lt;')}</pre>`;
            currentStreamDiv = null;
        } else {
            window.ideAppendMessage(event.payload.message, false);
        }
    });
    ...
    await listen('ide-tool-start', (event) => {
        ...
        let argsStr = '';
        try { argsStr = JSON.stringify(event.payload.args, null, 2); }
        catch (e) { argsStr = String(event.payload.args); }
